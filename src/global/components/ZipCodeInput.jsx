 ...
 class CustomZipCodeInput extends React.Component{
    ...
    handleOnblur = (event, addressName)=>{
        ...
        if(!zipCode) {
            ...
            .then((data)=>{
                dispatch(change(REDUX_FORM_NAME, `${addressName}.city`, data.data.city))
                dispatch(change(REDUX_FORM_NAME, `${addressName}.country`, data.data.country))
                dispatch(change(REDUX_FORM_NAME, `${addressName}.state`,  data.data.state))
            })
            .catch(err => {
                this.clearAddressFields(dispatch, addressName)
            })
    ...
    render() {
        ...
        return (
            <TextInput label="Zipcode" source={this.props.source} 
